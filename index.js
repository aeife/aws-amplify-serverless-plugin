...
class ServerlessAmplifyPlugin {
    ...
    toJavascriptConfiguration(configuration, appClient) {
        ...
        let authconfig = this.fetchAppClient(configuration, appClient);
        if (authconfig) {
            config_obj.aws_cognito_region = authconfig.Region;
            config_obj.aws_user_pools_id = authconfig.PoolId;
            config_obj.aws_user_pools_web_client_id = authconfig.AppClientId;
            config.push(
            ...
            if (authconfig.AppClientSecret) {
                config_obj.aws_user_pools_web_client_secret = authconfig.AppClientSecret;
        ...
        if (configuration.hasOwnProperty('CognitoIdentity')) {
            if (!authconfig) {
                config_obj.aws_cognito_region = configuration.CognitoIdentity.Region;
                config.push(`    "aws_cognito_region": "${configuration.CognitoIdentity.Region}",`);
            }
            config_obj.aws_cognito_identity_pool_id = configuration.CognitoIdentity.PoolId;
            config.push(`    "aws_cognito_identity_pool_id": "${configuration.CognitoIdentity.PoolId}",`);
        }

        if (configuration.hasOwnProperty('AppSync')) {
            config_obj.aws_appsync_graphqlEndpoint = configuration.AppSync.ApiUrl;
            config_obj.aws_appsync_region = configuration.AppSync.Region;
            config_obj.aws_appsync_authenticationType = configuration.AppSync.AuthMode;
            config.push(
        ...
        if (configuration.hasOwnProperty('S3TransferUtility')) {
            config_obj.aws_user_files_s3_bucket = configuration.S3TransferUtility.Bucket;
            config_obj.aws_user_files_s3_bucket_region = configuration.S3TransferUtility.Region;
            config.push(
        ...
        if (configuration.hasOwnProperty('APIGateway')) {
            config_obj.aws_cloud_logic_custom = [{
              "name": "api",
        ...
        console.log('constructed config file');
        ...
        console.log(constructed_config);
        config.push('};', '', 'export default aws_exports;', '');
        return config.join("\n");
    }
